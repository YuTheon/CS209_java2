    ...
    void testGetCourseListOfInstructor() {
        try {
            ...
            String expected = Files.readString(Paths.get("resources", "local_answer", "Q3.txt"),
                            StandardCharsets.UTF_8)
                    .replace("\r", "").strip();
//            List<String> ex = new ArrayList<>(Arrays.stream(expected.split("\n")).toList());
//            Collections.sort(ex);
//            String exp = ex.stream().collect(Collectors.joining("\n"));
//            Map<String, List<List<String>>> ex_map =
//
//            assertEquals(exp, mapToString(res));
